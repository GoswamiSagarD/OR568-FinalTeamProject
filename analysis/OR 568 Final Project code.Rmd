...
ggplot(df, aes(x=temperature)) + 
  ...
  theme_gray() + 
  facet_wrap(~season) +
  labs(x= 'Temperature', y= 'Bike Rentals', title='Bike Rentals by Temperature', caption="Temperature is measured in celcius") +
...
ggplot(data=df, aes(workhours)) + 
  ...
  labs(x= 'Working Hours', y ="Bike Rentals", title='Bike Rentals by Work/Non-Work Hours') +
  theme_gray() +
  theme(plot.title = element_text(hjust = 0.5, face='bold', size=18L))

# bike rentals based on holiday/no holiday 
ggplot(data=df, aes(holiday)) + 
  geom_bar(fill="steelblue", width=0.3) + 
  labs(x="", y ="Bike Rentals", title='Bike Rentals by Holiday/No Holiday') +
  theme_gray() +
  theme(plot.title = element_text(hjust = 0.5, face='bold', size=18L))
...
#sum bike counts by month/year 
sum_month <- df2 %>% 
  group_by(month = floor_date(date, 'month')) %>%
  summarise(sum_c = sum(bikecount))

#convert to data frame 
sum_month <- as.data.frame(sum_month)

#bar graph of bike rentals by month 
ggplot(sum_month, aes(x=month, y=sum_c)) +
  geom_point() + 
  geom_line(size = 1, color = "#4682B4", alpha=0.9) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  labs(x="", y = "Bike Rentals", title = "Bike Rental by Month", 
  caption = "Data: Seoul Bike Sharing") +
  theme_gray() +
  theme(plot.title = element_text(size = 18L, 
  face = "bold", hjust = 0.5))
...
for(j in 1:k){
  best.fit=regsubsets(bikecount~., data=df2[folds!=j,], nvmax=(ncol(df2)-1))
...
```
```{r}
barplot(order(pred.imp$Overall), 
        names.arg = row.names(pred.imp))
```
```{r}
pred.imp <- varImp(bag)
ba <- cbind(newColName = rownames(pred.imp), pred.imp)
rownames(ba) <- 1:nrow(ba)
t =ba[order(-Overall),]
barplot(order(t$Overall), 
        names.arg = t$newColName)
```
...
```{r}
model <- c(rep("Linear Regression" , 2) , rep("Random Forest" , 2) , rep("Boosting" , 2) , rep("Bagging" , 2), rep("Deep Learning" , 2))
sampletype <- rep(c("in-sample" , "out-of-sample") , 5)
value <- c(453.82, 415.71, 88.35, 195.24, 381.73, 388.31, 381.73, 175.52, 57.81, 154.7)
results <- data.frame(model, sampletype, value)

```

```{r}
ggplot(results, aes(fill=sampletype, y=value, x=model)) + 
  geom_bar(position="dodge", stat="identity") + 
  theme_gray() + 
  labs(x= '', y= 'RMSE', title='Model Performance by RMSE', caption="Data: Seoul Bike Sharing") +
  theme(plot.title = element_text(hjust = 0.5, face='bold', size = 18L), plot.caption = element_text(size=11)) + 
  theme(axis.text.x=element_text(angle=50, hjust=1, size = 10), axis.text.y=element_text(size = 10), axis.title = element_text(size = 10)) + 
  coord_flip()
