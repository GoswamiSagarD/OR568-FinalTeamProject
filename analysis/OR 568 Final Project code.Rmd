...
d = data.frame(test$bikecount, exp(pred.test))
write.csv(d, 'lm_test.csv')
```
...
d = data.frame(test$bikecount, rf.pred)
write.csv(d, 'rf_test.csv')
```
...
d = data.frame(test$bikecount, pred)
write.csv(d, 'boost_test.csv')
```
Train: 
MSE- 145717.1
RMSE- 381.73
...
t =baf[order(-Overall),]
barplot(t$Overall, 
        names.arg = t$newColName, main = 'Bagging Variable Importance', xlab = 'Variable', ylab = 'Importance')
```
...
d = data.frame(test$bikecount, pred.test)
write.csv(d, 'bag_test.csv')
```
...
Hypothesis testing 
```{r}
t.test(bikecount~holiday, data=df2)
...
We can consider conducting hypothesis testing on work hours instead of holiday 
```{r}
t.test(bikecount~workhours, data=df)

#h0: bike sharing is more common during non-workhours 
#h1: bike sharing is more common during workhours 
```
```{r}
