...
```{r}
reg.fit.best <- regsubsets(bikecount~., df, nvmax=14)
...
for(j in 1:k){
  best.fit=regsubsets(bikecount~., data=df[folds!=j,], nvmax=(ncol(df)-1))
  
  for(i in 1:(ncol(df)-1)){
    pred = predict(best.fit, df[folds==j,], id=i)
    cv.errors[j,i] = mean( (df$bikecount[folds==j]-pred)^2 )
  }
}
```
...
```{r}
fit_5f = lm(bikecount~.-visibility, data = df)# model from 5-Fold CV
summary(fit_5f)
```
...
```{r}
lm.train <- lm(bikecount~.-visibility, data=train)
lm.pred <- predict(lm.train, test)
mean((test$Apps - lm.pred)^2)
(lin_info <- postResample(lm.pred, test$bikecount))
