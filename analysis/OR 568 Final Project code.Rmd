...
lm2 <- lm(log(bikecount)~., data = train)
lm2.pred <- predict(lm2, test)

(lin_info <- postResample(lm2.pred, log(test$bikecount))) #with all variables
(lin_info <- postResample(lm.pred, log(test$bikecount)))  #with 10 variables
summary(lm.train)
summary(lm2)
...
Poisson Regression 
```{r}
df$bikecount[df$bikecount == 0.000001] <- as.numeric(0) #switching 0.000001 back to 0 

pois.reg <- glm(bikecount~.-workhours, family="poisson", data=df) #workhours isnt statistically significant 
summary(pois.reg)
...
Chi-square test 
```{r}
pchisq(2126348,8741, lower.tail=FALSE) #p-value = 0, which is smaller than the significance level of 0.05. We can conclude that the data doesn't fits the model reasonably well.
